...
The SQL Server JDBC driver needs to be configured in Admin UI. There are two popular SQL Server drivers available:

* Official Microsoft JDBC Driver
* JTDS Driver (open source)

Both of these drivers can be used to connect Exasol and SQL Server. The JTDS Driver generally has better performance over the official SQL Server driver, but your results may vary based on the specific use case. You can find more details in the [Best Practices for IMPORT/EXPORT with SQL Server](https://exasol.my.site.com/s/article/Best-Practices-for-IMPORT-EXPORT-with-SQL-Server) article. The JTDS Driver also doesn't seem to receive many updates anymore.
...
To enable Windows authentication to work properly, you need to disable "Security Manager". SQL Server requires this so that it can find some additional libraries. However, these libraries are not needed and you **do not need to upload the mssql-jdbc_auth-9.2.1.x64.dll file.**

To disable the Security Manager, add line "NOSECURITY=YES" to the driver's `settings.cfg` file (see also [Add JDBC driver](https://docs.exasol.com/db/latest/administration/on-premise/manage_drivers/add_jdbc_driver.htm)).
...
```sql
import from jdbc at JDBC_SQLSERVER statement 'select 1';
```
...
If you receive the following error message, then the Security Manager needs to be disabled for your JDBC connection in settings.cfg:
