...
--/
CREATE OR REPLACE PYTHON  ADAPTER SCRIPT ADAPTER_SCHEMA."RLS_ADAPTER" AS
import json
import string
import glob
...
def get_meta_for_schema(cn, s):
    ...
    with EXASOL.connect(c.address, c.user, c.password) as connection:
        with connection.cursor() as cursor:
            ...
            cursor.execute("select table_name from EXA_ALL_TABLES where table_schema='"+s+"' union all  select view_name from EXA_ALL_VIEWS where view_schema ='"+s+"' ;")
            if cursor.rowcount == 0: raise ValueError('Config error: Can not create adapter for empty schema')
            for row in cursor:
                tabs.append(row[0])
        ...
        for t in tabs:
            with connection.cursor() as cursor:
                cursor.execute("select column_name, column_type, column_maxsize, column_num_prec, column_num_scale from EXA_ALL_COLUMNS where column_schema='"+s+"' and column_table='"+t+"';")
                cols=[]
                for row in cursor:
...
def user_is_restricted(username,cn,t):
    ...
    with EXASOL.connect(c.address, c.user, c.password) as connection:
        with connection.cursor() as cursor:
            cursor.execute("select distinct restricted from ADAPTER_SCHEMA.RESTRICTIONS_ROWS WHERE USERNAME IN(select granted_role from EXA_DBA_ROLE_PRIVS connect by grantee= PRIOR granted_role start with grantee='"+username+"') OR username='"+username+"' AND TABLENAME='"+t+"'")
            if cursor.rowcount == 0: return bool(0)
            if cursor.rowcount > 1: raise ValueError('Config error: conflicting restrictions for user '+username+' in adapter_schema.RESTRICTIONS_ROWS')
            return bool(1)
...
def get_restricted_columns_for_table(cn, s, tbl, u):
    ...
    with EXASOL.connect(c.address, c.user, c.password) as connection: 
        with connection.cursor() as cursor:
            cursor.execute("SELECT distinct columnname from adapter_schema.RESTRICTIONS_COLUMNS where username in (select granted_role from EXA_DBA_ROLE_PRIVS connect by grantee= PRIOR granted_role start with grantee='"+u+"') or username='"+u+"' and tablename='"+tbl+"'")
            res = []
...
def get_columns_for_table(cn, s, tbl, u):
    ...
    with EXASOL.connect(c.address, c.user, c.password) as connection: 
        with connection.cursor() as cursor:
            if l is not None: cursor.execute("select case when column_name  in ("+l+") then '''***''' else column_name end from EXA_ALL_COLUMNS where column_schema='"+s+"' and column_table='"+tbl+"'")
            else: cursor.execute("select column_name from EXA_ALL_COLUMNS where column_schema='"+s+"' and column_table='"+tbl+"'")
            cols = []
            for row in cursor:
                cols.append(row[0])
        return ','.join(cols)
